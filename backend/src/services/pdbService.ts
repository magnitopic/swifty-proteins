...
function convertMmcifToPdb(
  mmcifText: string,
  options: MmcifToPdbOptions = { includeConect: true }
): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (
      atomSiteHeaders.length > 0 &&
      (line.startsWith("HETATM") || line.startsWith("ATOM"))
    ) {
  ...
  for (let i = atomDataStartIndex; i < lines.length; i++) {
    ...
    pdbLines.push(
      `${record}${serial} ${name} ${resName} ${chainId}${resSeq}    ${x}${y}${z}${occ}${temp}${element}`
    );
  }
...
async function getPdbFile(ligandId: string) {
  ...
  console.log("pdb:", pdb);
  // typeof pdb is string
  console.log(typeof pdb);
